...
rawtype bigmul (rawtype *d, rawtype *s, rawtype f, size_t n)
{
    ...
    {
        rl = *s * f;
        rh = (rawtype) (invbase * ((double) *s * (double) f - (double) (rl & 0x80000000)));
...
rawtype bigdiv (rawtype *d, rawtype *s, rawtype f, size_t n)
{
    size_t t;
    rawtype b, r, rl, rh;
    double invf = (1.0 - 2.0 * eps) / (double) f;       // Round down
    ...
    for (t = 0; t < n; t++)
    {
        ...
        r = (rawtype) (invf * ((double) *s + base * (double) b));

        rl = r * f;
        rh = (rawtype) (invbase * ((double) r * (double) f - (double) (rl & 0x80000000)));
