...
{
    if(s.size() % 2 != 0)
        THROW_NEW_GUTIL_EXCEPTION2( Exception, "Input string was not in the correct format" );
...
char Utils::StringHelpers::charToHex(char c)
{
    ...
    {	// if it's A-F
        ...
        THROW_NEW_GUTIL_EXCEPTION2( Exception, "Unrecognized hex character" );
...
char Utils::StringHelpers::hexToChar(char c)
{
    ...
    if(0 != (c & 0xf0))
        THROW_NEW_GUTIL_EXCEPTION2( Exception, "Unrecognized hex digit" );
