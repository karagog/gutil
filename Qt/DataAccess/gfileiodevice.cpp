...
GFileIODevice::GFileIODevice(const QString &filename, QObject *parent)
...
void GFileIODevice::send_data(const QByteArray &data)
{
    lock_file(true);
    _open_file(true);
...
QByteArray GFileIODevice::receive_data()
{
    lock_file(false);
    _open_file(false);

    QByteArray ret = GQIODevice::receive_data();
...
bool GFileIODevice::has_been_updated() const
{
    return _last_update_time != QFileInfo(FileName()).lastModified();
}

bool GFileIODevice::has_data_available()
{
    return has_been_updated();
}

void GFileIODevice::SetFileName(const QString &filename)
{
...
QByteArray GFileIODevice::FileData()
{
    return ReceiveData();
}

void GFileIODevice::TruncateFile()
{
    FailIfReadOnly();
...
void GFileIODevice::_open_file(bool for_write)
{
...
void GFileIODevice::_close_file()
{
    File().close();
}

void GFileIODevice::lock_file(bool write)
{
    if(write)
...
void GFileIODevice::unlock_file()
{
