...
DatabaseIODevice::~DatabaseIODevice()
{
    if(_selection_parameters)
        delete _selection_parameters;
}

bool DatabaseIODevice::IsReady() const
{
    return QSqlDatabase::database(_connection_id).isOpen();
}

void DatabaseIODevice::CreateTable(const QString &name,
...
bool DatabaseIODevice::DropTable(const QString &name)
{
    _fail_if_not_ready();
...
QString DatabaseIODevice::CreateIndex(const QString &table_name,
...
void DatabaseIODevice::DropIndex(const QString &index_name)
{
    _fail_if_not_ready();
...
void DatabaseIODevice::Insert(const VariantTable &t)
{
    _fail_if_not_ready();
...
VariantTable DatabaseIODevice::Select(
...
VariantTable DatabaseIODevice::Select(const QString &table_name,
                                       const QStringList &columns)
{
    QList<int> cols;
    {
...
VariantTable DatabaseIODevice::Select(const DatabaseSelectionParameters &params,
...
long DatabaseIODevice::Count(const DatabaseSelectionParameters &params)
{
    _fail_if_not_ready();
...
long DatabaseIODevice::Count(const QString &table_name)
{
    return Count(GetBlankSelectionParameters(table_name));
}

void DatabaseIODevice::Update(const DatabaseSelectionParameters &sp,
...
void DatabaseIODevice::Delete(const DatabaseSelectionParameters &p)
{
    _fail_if_not_ready();
...
DatabaseSelectionParameters DatabaseIODevice::GetBlankSelectionParameters(
...
DatabaseValueParameters DatabaseIODevice::GetBlankValueParameters(const QString &table_name) const
{
...
VariantTable DatabaseIODevice::GetBlankTable(const QString &table_name) const
{
    VariantTable *t(0);
    if(_tables.contains(table_name))
...
void DatabaseIODevice::send_data(const QByteArray &d)
{
...
QString DatabaseIODevice::_prepare_where_clause(const Vector<String> &column_keys,
...
QByteArray DatabaseIODevice::receive_data()
{
    _p_ReturnValue.clear();
...
bool DatabaseIODevice::has_data_available()
{
    // We can always query the database for more data
    return true;
}

QByteArray DatabaseIODevice::prepare_send_data(WriteCommandsEnum cmd,
...
void DatabaseIODevice::OpenDatabaseConnection()
{
...
void DatabaseIODevice::CloseDatabaseConnection()
{
    QSqlDatabase db(QSqlDatabase::database(GetDatabaseConnectionId()));
    if(db.isValid())
        db.close();
...
void DatabaseParametersBase::ReadXml(QXmlStreamReader &sr)
{
    ...
    for(int i = 0; i < col_opts->ColumnCount(); i++)
        ColumnOptions().Add((DatabaseIODevice::ColumnOption)(*col_opts)[i]->toInt());
}
