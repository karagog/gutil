...
QMutex GConsoleIODevice::global_console_mutex;

GConsoleIODevice::GConsoleIODevice(QObject *parent)
    :GIODevice(parent)
{
    _engaged = false;
...
GConsoleIODevice::~GConsoleIODevice()
{
    Disengage();
}

void GConsoleIODevice::SetEngaged(bool engage)
{
    if(engage)
    {
        if(_engaged)
...
bool GConsoleIODevice::has_data_available()
{
    bool ret;
    _messages_lock.lock();
    {
...
bool GConsoleIODevice::_has_data_available_locked() const
{
    return _messages_received.length() > 0;
}

void GConsoleIODevice::run()
{
    QString buf;
...
void GConsoleIODevice::WriteLine(const QString &data)
{
...
QString GConsoleIODevice::ReadLine()
{
    return ReceiveData();
}

void GConsoleIODevice::send_data(const QByteArray &d)
{
...
QByteArray GConsoleIODevice::receive_data()
{
    QByteArray ret;
    _messages_lock.lock();
    {
...
void GConsoleIODevice::_fail_if_not_initialized()
{
    if(!_engaged)
